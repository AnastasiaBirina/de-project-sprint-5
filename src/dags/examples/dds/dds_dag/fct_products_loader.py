...
from examples.dds.dds_dag.orders_loader import OrderDdsRepository
from examples.dds.dds_dag.products_loader import ProductDdsObj, ProductDdsRepository
...
class BonusEventRepository:
    ...
    def load_raw_events(self, conn: Connection, pay_event: str, last_loaded_id: str) -> Optional[EventObj]:
        with conn.cursor() as cur:
            cur.execute(
                """
                    SELECT id, event_ts, event_type, event_value
                    FROM stg.bonussystem_events
                    WHERE event_type = (%(pay_event)s) and id > (%(last_loaded_id)s)
                """,
                {
    ...
    def load_product_facts(self):
        with self.dwh.connection() as conn:
            ...
            for payment_raw in load_queue:
                payment_obj = BonusPaymentJsonObj(json.loads(payment_raw[3]))
                print("here 123", str(payment_obj))
                ...
                wf_setting.workflow_settings[self.LAST_LOADED_ID_KEY] = payment_raw[0]
